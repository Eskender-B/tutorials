...
char* indexes(const char* path)
{
	...
	//search for index.php or index.html
	//start from bottom so that index.php is reached first if it exists
	for(int i = n - 1; i >= 0; i--)
	{
		...
		{
			ret_path = malloc(strlen(path) + 10);
			if(ret_path == NULL)
			{	
				free(ret_path);
				freedir(entries, n);
		...
		{
			ret_path = malloc(strlen(path) + 11);
			if(ret_path == NULL)
			{	
				free(ret_path);
				error(500);
...
const char* lookup(const char* path)
{
    ...
    next1:
    needle = strcasestr(haystack, ".css");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "text/css";
        else
        {   
		haystack = needle + 4;
    ...
    needle = NULL;
    next2:
    needle = strcasestr(haystack, ".html");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 5)
            return "text/html";
        else
        {   
            haystack = needle + 5;
    ...
    needle = NULL;
    next3:
    needle = strcasestr(haystack, ".gif");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "image/gif";
    ...
    needle = NULL;
    next4:
    needle = strcasestr(haystack, ".ico");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "image/x-icon";
    ...
    needle = NULL;
    next5:
    needle = strcasestr(haystack, ".jpg");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "image/jpeg";
    ...
    needle = NULL;
    next6:
    needle = strcasestr(haystack, ".js");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 3)
    ...
    needle = NULL;
    next7:
    needle = strcasestr(haystack, ".php");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "text/x-php";
    ...
    needle = NULL;
    next8:
    needle = strcasestr(haystack, ".png");
    if (needle != NULL)
    {
        if ( needle-haystack == strlen(haystack) - 4)
            return "image/png";
...
{
	//ensure line is in the form: method sp request-target sp version CRLF
    ...
    //ensure line is properly null terminated within LimitRequestLine + 1 characters
	const char* next = line;
	int count = 0;
	while(*next != '\0')
	{
		count++;
		if(count > LimitRequestLine)
		{
			error(500);
			return false;
		}
	...
	if((needle - haystack) > 3)
	{
		//method is more than 3 characters
		error(405);
		return false;
	}

	strncpy(method, haystack, 3);
    method[3] = '\0';	
	...
	//separate request-target into abs_path?query
	needle = NULL;
	...
	else
	{
		...
		if(request_target[strlen(request_target)] == '?')
		{
			strncpy(abs_path, request_target, strlen(request_target) - 1);
			abs_path[strlen(request_target)-1] = '\0';
			*query = '\0';
			return true;
		}
		else
		{
			strncpy(abs_path, request_target, needle - haystack);
			abs_path[needle - haystack] = '\0';
			strcpy(query, needle + 1);
			query[strlen(request_target)-(needle-haystack)-1] = '\0';
			return true;
		}
	}
